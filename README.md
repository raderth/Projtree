![header](https://github.com/raderth/Projtree/blob/main/logo.png?raw=true)

<h4>Project management and planning that makes *some* sense</h4>

<h1>Setup</h1>
<ul>
  <li>Run app.py</li>
  <li>Go to the /setup_db page, this will create a default user username: admin, password: admin123</li>
</ul>

<h1>Usage</h1>
<p>First create a parent task, you can then add dependencies which are basically tasks that need to be completed before this task can be done. The first parent task should be something like "project completion"</p>
